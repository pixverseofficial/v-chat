...
export default function Dashboard() {
  ...
  return (
    <div className="h-screen w-full bg-[#f4f7fb] flex overflow-hidden font-sans text-slate-700 relative">
      ...
      {/* --- COLUMN 1: LEFT SIDEBAR (Slim) --- */}
      <div className="w-20 bg-white border-r border-slate-200 flex flex-col items-center py-8 z-40">
        ...
        <div className="flex flex-col gap-8 flex-1 mt-4">
          <button 
            ...
            className={`p-3 rounded-xl transition-all ${activeTab === 'chats' ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-400 hover:text-blue-600'}`}
          >
          ...
          <button 
            ...
            className={`p-3 rounded-xl transition-all relative ${activeTab === 'friends' ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-400 hover:text-blue-600'}`}
          >
            <Users className="w-6 h-6" />
            {pendingRequests.length > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border-2 border-white"></span>}
          </button>
          
          <button 
            onClick={() => setActiveTab('explore')} 
            className={`p-3 rounded-xl transition-all ${activeTab === 'explore' ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-400 hover:text-blue-600'}`}
          >
          ...
          <button 
            ...
            className={`p-3 rounded-xl transition-all ${isSettingsOpen ? 'bg-blue-50 text-blue-600' : 'text-slate-400 hover:text-blue-600'}`}
          >
            <Settings className="w-6 h-6" />
          </button>
        </div>

        <button onClick={() => signOut(auth)} className="p-3 text-slate-400 hover:text-red-500 transition-all mt-auto pt-6">
          <LogOut className="w-6 h-6" />
        </button>
      </div>

      {/* --- COLUMN 2: MIDDLE LIST AREA --- */}
      <div className="w-96 bg-white border-r border-slate-200 flex flex-col z-30">
        <div className="p-6 pb-2">
          ...
          <div className="relative">
            ...
            <form onSubmit={handleSearch}>
              <input type="text" placeholder="Search people..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-100 border-none py-3 pl-10 pr-4 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500/10" />
            </form>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 mt-4 space-y-2">
          ...
          {(activeTab === 'chats' || activeTab === 'friends') && friends.map(f => (
            <div 
              ...
              className={`flex items-center p-3 rounded-2xl cursor-pointer transition-all ${selectedChat?.id === f.id && activeTab === 'chats' ? 'bg-blue-600 text-white shadow-xl' : 'hover:bg-slate-50'}`}
            >
              <div className="w-12 h-12 rounded-full overflow-hidden flex items-center justify-center font-bold bg-slate-100 text-blue-600 border border-white shadow-sm">
              ...
              <div className="ml-4 flex-1">
                ...
                <p className={`text-[10px] mt-1 font-bold ${selectedChat?.id === f.id && activeTab === 'chats' ? 'text-blue-100' : 'text-green-500'}`}>ONLINE</p>
              </div>
            </div>
          ))}
          ...
          {activeTab === 'friends' && pendingRequests.map(r => (
             <div key={r.id} className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between mt-4 border border-slate-200">
      ...
      <div className="flex-1 bg-white flex flex-col relative z-20">
        {selectedChat && activeTab === 'chats' ? (
          <>
            ...
            <div className="h-20 border-b border-slate-100 flex items-center justify-between px-8 bg-white/90 backdrop-blur-md">
              ...
              <div className="flex items-center gap-5 text-slate-400">
                <button className="hover:text-blue-600 transition-all"><Phone className="w-5 h-5" /></button>
                <button className="hover:text-blue-600 transition-all"><Video className="w-5 h-5" /></button>
                <button className="hover:text-blue-600 transition-all"><Info className="w-5 h-5" /></button>
              </div>
            </div>
            ...
            <div className="p-6 bg-white border-t border-slate-100">
              <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-100 shadow-inner">
                ...
                <form onSubmit={sendMessage} className="flex-1 flex items-center gap-4">
                  <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type your message here..." className="flex-1 bg-transparent border-none py-2 text-sm outline-none text-slate-700" />
                  <button type="submit" className="bg-blue-600 p-3 rounded-xl text-white shadow-lg shadow-blue-100 hover:scale-105 transition-all"><Send className="w-5 h-5" /></button>
                </form>
              </div>
            </div>
          </>
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-slate-50/50">
            ...
            <p className="text-slate-400 text-xs mt-2 font-bold uppercase tracking-widest">Connect with friends to start chatting</p>
          </div>
        )}
      </div>

      {/* --- SETTINGS MODAL (Fully Corrected) --- */}
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 relative shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="flex flex-col items-center gap-8">
              ...
              <button onClick={() => setIsSettingsOpen(false)} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all uppercase tracking-widest">Done</button>
            </div>
          </div>
        </div>
      )}
